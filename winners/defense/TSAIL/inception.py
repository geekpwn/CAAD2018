...
class Inception3(nn.Module):
    ...
    def __init__(self, denoise, shrink, noise, num_classes=1001):
    ...
    def forward(self, x, defense = False):
        ...
        if defense:
            ...
            r = torch.autograd.Variable(self.noise * (torch.rand(x.size()[0],x.size()[1],x.size()[2],x.size()[3]).cuda()-0.5),
             requires_grad=False)
            x = x + r
        outputs.append(x)
        # 299 x 299 x 3
...
class Net(nn.Module):
    def __init__(self, input_size, block, fwd_out, num_fwd, back_out, num_back, n, shrink, noise, hard_mining = 0, loss_norm = False):
        super(Net, self).__init__()
        denoise = Denoise(input_size[0], input_size[1], block, 3, fwd_out, num_fwd, back_out, num_back)
        self.net = Inception3(denoise,shrink,noise)
...
def get_net(input_size, block, fwd_out, num_fwd, back_out, num_back, n, shrink = 1, noise = 0, hard_mining = 0, loss_norm = False):
