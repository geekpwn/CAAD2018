...
class ResNet(nn.Module):
    def __init__(self, net_type, input_size, denoise_block, fwd_out, num_fwd, back_out, num_back, shrink, noise,
                 num_classes=1000):
        ...
        for i in range(len(block)):
            if block[i] is None:
                ...
                denoise.append(
                    Denoise(h_in[i], w_in[i], block[i], n_in[i], fwd_out[i], num_fwd[i], back_out[i], num_back[i]))
    ...
    def forward(self, x, defense=False):
        ...
        out = x
        if defense:
            out = self.shrink * self.denoise[0](out)
            r = torch.autograd.Variable(
                self.noise * (torch.rand(out.size()[0], out.size()[1], out.size()[2], out.size()[3]).cuda() - 0.5),
                requires_grad=False)
            out = out + r
        outputs.append(out)
        out = self.conv1(out)
...
class Net(nn.Module):
    def __init__(self, net_type, input_size, block, fwd_out, num_fwd, back_out, num_back, n, shrink, noise,
                 hard_mining=0, loss_norm=False):
        super(Net, self).__init__()
        self.net = ResNet(net_type, input_size, block, fwd_out, num_fwd, back_out, num_back, shrink, noise)
...
def get_net(net_type, input_size, block, fwd_out, num_fwd, back_out, num_back, n, shrink=1, noise=0, hard_mining=0,
            loss_norm=False):
    net = Net(net_type, input_size, block, fwd_out, num_fwd, back_out, num_back, n, shrink, noise, hard_mining,
              loss_norm)
