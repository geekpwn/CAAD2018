...
tf.flags.DEFINE_float(
    'prob', 0.4, 'probability of using diverse inputs.')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    target_class_onehot = tf.one_hot(target_class_input, num_classes)

    i = tf.constant(0)
    grad = tf.zeros(shape=batch_shape)
    x_adv, _, _, _, _, _ = tf.while_loop(stop, graph, [x_input, target_class_onehot, i, x_max, x_min, grad])
